...
class FNNDataLoader(object):
    ...
    def __init__(self, label_map, tokenizer, args, inputs, filenames, labels, aux_info, user_embeds, test=False,
                 batch_size=64):
        self.cuda = args.cuda
        ...
        assert self.user_embeds.shape[0] == len(self.inputs)
        self.test = test
    ...
    def load_Rs(self):
        ...
        # sim_topics = np.zeros((len(aux_info), self.evi_num, self.evi_num))
        # twt_weight_mat = np.zeros((len(aux_info), self.sent_num, self.evi_num))

        # target_li = [R_p, R_k, R_u, sim_topics, twt_weight_mat]
        ...
        # sim_topics_tensor = torch.FloatTensor(sim_topics)
        # twt_weight_mat_tensor = torch.FloatTensor(twt_weight_mat)
    ...
    def next(self):
        if self.step < self.total_step:
            ...
            # aux_info = self.load_Rs() if not self.args.case_study else None
